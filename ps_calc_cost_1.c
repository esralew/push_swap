...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/30 15:36:31 by elewin            #+#    #+#             */
/*   Updated: 2026/04/01 18:41:30 by elewin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

// if flag gets set to 1,
// then the next element in stack a will be moved through rotations 
// (upwards travel) if flag stays set to 0, then the next element will 
// be moved through reverse rotations (downwards travel)
...
t_list	*find_succ(t_list *node, t_list **stack_b)
{
	...
	if ((*((int *)(curr->content)) < val_node)
		&& (val_node < *(int *)ft_lstlast(*stack_b)->content))
		return (curr);
	while (1)
	{
